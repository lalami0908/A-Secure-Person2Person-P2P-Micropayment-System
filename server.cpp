...
void LoadCertificates(SSL_CTX* ctx, char* CertFile, char* KeyFile){
  ...
  //
    ...
    //New lines - Force the client-side have a certificate制定證書驗證方式
    ...
    if(newSocket > 0)
    {
      ...
      while(SSL_read(ssl[newSocket] , client_message ,sizeof(client_message)) > 0)
      {
        ...
        else
        {
          ...
          if (userID != -1  && users[userID].isOnline() == false) //already login
          {
  ...
  for (int i = 0; i < 10; ++i)
  {
    ...
    while(1)
    {
      ...
      while((client_fd = accept(serverSocket, (struct sockaddr *) &clientInfo, &addr_size))) 
      {
        ...
        /// SSL connect //把已經驗證過的ctx傳進去SSL_new()  
        ...
        {
          string success = "Connected accepted. Welcome to my server.\n";
